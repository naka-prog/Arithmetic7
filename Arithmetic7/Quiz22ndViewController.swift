...
class Quiz22ndViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let resultVC = segue.destination as? Result22ndViewController {
    ...
    @IBAction func nextButton(_ sender: UIButton) {
        ...
        if questionNumber >= totalNumber {
            ...
            result.append(resultLine)
            performSegue(withIdentifier: "Result22ndView", sender: nil)
        } else {
            // 計算処理を行う
            setQuestions()
        }
